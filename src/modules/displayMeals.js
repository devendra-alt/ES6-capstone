...
import displayLikes from './displayLikes.js';async function displayMeals(reservation) {
  ...
  const mainSection = document.querySelector('.section');  meals.forEach(async (meal) => {
    ...
    const likeCounter = document.createElement('p');    const commentBtn = document.createElement('button');
    const reservationBtn = document.createElement('button');    mealElement.setAttribute('data-mealID', meal.idMeal);
    ...
    reservationBtn.setAttribute('data-mealID', meal.idMeal);    imgEl.src = new URL(meal.strMealThumb);
    ...
    reservationBtn.classList.add('meal-item-reservation-btn');    reservationBtn.addEventListener('click', () => {
    ...
    mealElement.appendChild(reservationBtn);    mainSection.appendChild(mealElement);
  });
